...
export default function TimelinePage() {
  ...
  const timelineItems: TimelineItem[] = [
    ...
    ...notes.map((note) => {
      const metaParts = [];
      if (note.mood) metaParts.push(`Mood: ${note.mood}`);
      if (note.weather) metaParts.push(`Weather: ${note.weather}`);
      return {
        date: note.date,
        title: note.title,
        href: `/notes/${note.id}`,
        type: 'note' as const,
        meta: metaParts.length > 0 ? `${metaParts.join(' / ')} / Note` : 'Note',
    ...
    <div className="max-w-4xl mx-auto py-10 px-4">
      <header className="mb-12">
        ...
        <h3 className="text-lg text-zinc-500 dark:text-zinc-400">
          {timelineItems.length} items published so far
        </h3>
      </header>
      ...
      <div className="mb-12 p-6 rounded-2xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800">
        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">
          Today is day {dayOfYear} of {now.getFullYear()}.
        </p>
        <div className="space-y-2 text-sm text-zinc-500 dark:text-zinc-400">
          <p>{yearProgress}% of the year has passed</p>
          <p>{dayProgress}% of the day has passed</p>
        </div>
        <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-4 italic">
          Keep building while the days are moving.
        </p>
      </div>
